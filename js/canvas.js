...
function buildAppCanvas(){
	...
	for(n=0;n<maxFrame;n++){
		...
		if(n==0){
			frameWidth = ($.frameObj['i'+n].image.naturalWidth * (maxFrame-1)) + (frameSpace * (maxFrame-1));
		...
		frameStartX += $.frameObj['i'+n].image.naturalWidth + frameSpace;
	...
	btnSwitch = new createjs.Bitmap(loader.getResult("btnSwitch"));
	...
	btnSwitch.x = canvasW/100 * 90;
	btnSwitch.y = canvasH/100 * 66;
	...
	watermark = new createjs.Bitmap(loader.getResult("watermark"));
	centerReg(watermark);
	
	//loader
	stickerLoaderTxt = new createjs.Text();
	stickerLoaderTxt.font = "35px pacificoregular";
	stickerLoaderTxt.color = "#fff";
	stickerLoaderTxt.textAlign = "center";
	stickerLoaderTxt.textBaseline='alphabetic';
	stickerLoaderTxt.text = '';
	stickerLoaderTxt.x = canvasW/2;
	stickerLoaderTxt.y = canvasH/100 * 65;
	
	loaderContainer.addChild(bgLoader, textLoader);
	transformIconContainer.addChild(btnZoomIn, btnZoomOut, btnDone, btnRemove, btnRotateLeft, btnRotateRight);
	...
	cameraOptionContainer.addChild(titleCameraUpload, btnTakephoto, btnCancel, btnSwitch, btnStartCamera, textCameraAllow, textCameraDenied, textCameraNotSupport, textNoCamera);
	...
	mainContainer.addChild(logo, stickerLoaderTxt, btnCamera, btnUpload);
